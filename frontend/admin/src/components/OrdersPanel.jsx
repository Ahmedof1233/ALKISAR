...
function OrderCard({ order, onStatusChange, updating }) {
  ...
  return (
    <div className="card animate-slide-in hover:border-white/10 transition-all duration-300 group">
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs bg-white/5 text-gray-400 px-2 py-0.5 rounded-lg font-mono">
          ...
          {order.customer_phone && (
            <p className="text-gray-500 text-sm mt-0.5">📞 {order.customer_phone}</p>
          )}
          {order.customer_address && (
            <p className="text-gray-500 text-sm mt-0.5">📍 {order.customer_address}</p>
          )}
        </div>
        <div className="text-left">
          <div className="text-brand-400 font-bold text-xl">{order.total_amount} ج.م</div>
      ...
      {/* Items */}
      <div className="space-y-1.5 mb-4 bg-dark-700/50 rounded-xl p-3">
        {items.map((item, i) => (
          <div key={i} className="flex justify-between items-center text-sm">
            <span className="text-gray-300">{item.name}</span>
            <div className="flex items-center gap-3">
              <span className="text-gray-500">×{item.qty}</span>
              <span className="text-gray-400">{item.price * item.qty} ج.م</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Notes */}
      {order.notes && (
        <div className="bg-yellow-500/5 border border-yellow-500/10 rounded-xl px-3 py-2 mb-4">
          <p className="text-yellow-400/80 text-sm">📝 {order.notes}</p>
        </div>
      )}

      {/* Average Time Tag */}
      {STATUS_MAP[order.status]?.avgTime && (
        <div className="flex items-center gap-1.5 text-xs text-gray-400 bg-white/5 rounded-lg px-2.5 py-1.5 mb-3 w-fit">
          <span>⏱️</span>
          <span>متوسط المرحلة: <strong>{STATUS_MAP[order.status]?.avgTime}</strong></span>
        </div>
      )}
        ...
        <div className="flex gap-2 pt-3 border-t border-white/5">
          ...
          <button
            onClick={() => {
              ...
              const itemsRows = order.items.map(it => `
                <tr>
                  ...
                  <td style="text-align:center; padding:4px 0; border-bottom:1px dashed #ccc;">x${it.quantity}</td>
                  <td style="text-align:left; padding:4px 0; border-bottom:1px dashed #ccc;">${it.price * it.quantity} ج.م</td>
                </tr>
              `).join('');
              ...
              printWin.document.write(`
                ...
                <body>
                  ...
                  <div><strong>العنوان:</strong> ${order.address || 'استلام من الفرع'}</div>
            ...
            className="px-3 py-2 bg-dark-700 hover:bg-dark-600 text-gray-300 hover:text-white rounded-xl text-sm font-medium border border-white/10 transition-colors flex items-center justify-center gap-1"
          >
            <span>🖨️</span>
          </button>

          {order.status === 'delivered' && (
            <div className="flex-1 flex items-center justify-center gap-2 bg-green-500/10 border border-green-500/20 rounded-xl py-2 text-green-400 font-semibold">
              <span>✅</span>
              <span>تم تسليم الطلب بنجاح</span>
            </div>
          )}
        </div>
      </div>
...
function FilterTab({ label, count, active, onClick, color }) {
  ...
  return (
    ...
    >
      {label}
      {count !== undefined && (
        <span className={`mr-2 px-1.5 py-0.5 rounded-full text-xs bg-white/10`}>{count}</span>
      )}
    </button>
  );
}
...
export default function OrdersPanel() {
  ...
  return (
    <div className="animate-fade-in">
      {/* Live Notification */}
      {liveMsg && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-dark-600 border border-brand-500/30 text-white px-6 py-3 rounded-2xl shadow-2xl animate-slide-in flex items-center gap-3">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">تحديثات فورية عبر SSE ومتابعة مراحل التحضير والتوصيل</p>
        </div>
        <button
      ...
      {/* Filter Tabs */}
      <div className="flex gap-2 mb-6 bg-dark-800 p-1.5 rounded-2xl w-fit flex-wrap">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[1,2,3].map(i => (
            <div key={i} className="card h-52 animate-pulse bg-dark-700" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
